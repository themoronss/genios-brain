...
| 5 | [Context Reasoner](layer-2/step-05-context-reasoner.md) | **NEXT · and it now carries THREE deferrals** | the one new model site · **per situation, not per event**. ⛔ L2-2 left it four fields + migration 0182 + two validators; L2-3 left it slice persistence; and `model_writable_fields()`, `proposal_gate` and `SLICE_TOKEN_BUDGET` were all built FOR it and must be used rather than rebuilt. Collected at the top of its step file | — |
