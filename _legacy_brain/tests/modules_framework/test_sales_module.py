...
def test_sales_manifest_loads() -> None:
    ...
    assert m.provides.rules == ["pricing", "churn", "velocity", "competition", "qualification", "signals"]
...
def test_sales_module_loads_into_package() -> None:
    ...
    # 6 rule files (churn, velocity, pricing, competition, qualification, signals)
    assert len(pkg.ruleset.rules) >= 18  # deepened from 10 -> 20 (MEDDIC) -> 26 (signal-driven)
