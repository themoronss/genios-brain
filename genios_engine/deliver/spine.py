...
def materialize(conn, obj: DeliveryObject, *, at: datetime, card_id: str | None = None,
    ...
    result = conn.execute(text(
        ...
        " recipient, band, channel_class, interrupt, execution_id, execution_hash, audience, "
        " destination, fmt, priority, daily_budget, source, route_ladder, route_cursor, "
        " retry_generation, lifecycle, dedupe_key, authority_expires_at, legacy_reconcile) "
        "values (:id, :o, :did, :card, :ch, :pl, 'queued', 0, :at, :rcp, :band, :cc, :intr, "
