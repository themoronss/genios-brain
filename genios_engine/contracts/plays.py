...
Boundary VOCABULARY only (stdlib; no database, no layer import) so every layer may use it: the
play ids, the schemas (`additionalProperties:false` everywhere), a STRICT validator, the words a
human approves, the frozen §3.1 request bytes and the §3.5 `delegate` action shape. The agent
lookup lives in `platform/agent_plays.py`; evidence filtering and the verbs in `executive/`.

`jsonschema` is a dev-only dependency (pyproject), so production always runs the built-in check;
when `jsonschema` is importable it runs as well and its errors are added, never substituted.
"""
...
__all__ = ["PLAYS", "PLAY_FOLLOW_UP", "PLAY_REASSIGN", "PLAY_RESCHEDULE", "SCHEMAS",
           "body_sha256", "delegate_action", "instruction", "iso_utc", "request_body",
           "request_document", "reschedule_windows", "summary", "validate"]
