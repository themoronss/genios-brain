...
| **Adaptive** — what actually worked | `lvl3_config.scoring_defaults.rule_offsets` + `rule_mutes` | Layer 6 Learning, through `write_lvl3_offset` only | the merge |
...
map holding in practice: Layer 6 Learning never imports Layer 3 or Layer 4 — it writes a row that the
