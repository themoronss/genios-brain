...
def test_the_capability_is_shadow_locked_until_activation(orchestrator):
    """Deployment runbook Lock 2: the shipped v2 manifest is NOT delivery-enabled. The mechanism is
    proven by flipping only that flag — exactly what activation Step 5.2 does — and confirming the
    same LIVE run then authorizes delivery. The shipped manifest itself stays shadow-locked."""
    from dataclasses import replace

    assert DEAL_COOLING_FULL_V2.live_delivery_enabled is False
    deliverable = replace(DEAL_COOLING_FULL_V2, live_delivery_enabled=True)
    activated = orchestrator.execute(_request(deliverable))
    assert activated.request.mode == ExecutionMode.LIVE
    assert activated.decision.outcome == DecisionOutcome.DECISION
    assert activated.authorizes_delivery is True
