...
# The shipped v2 manifest is shadow-locked (`live_delivery_enabled=False`, deployment runbook
# Lock 2). The publication seam only acts on an *authorized* decision, so exercising its logic
# needs a delivery-enabled capability. This local variant flips only that flag — exactly what
# activation Step 5.2 does — so these tests prove the seam without unlocking the shipped manifest.
DELIVERABLE_V2 = replace(DEAL_COOLING_FULL_V2, live_delivery_enabled=True)
...
def execution(orchestrator):
    return orchestrator.execute(_request(DELIVERABLE_V2))
...
def test_native_rule_ids_covers_every_scheduled_capability():
    # Lock 1 (deployment runbook): only the v1 baseline is scheduled today; v2 is imported but not
    # yet in the sweep roster. Activation Step 1 adds it, at which point this becomes both ids.
    assert native_rule_ids(BUILTIN_CAPABILITIES) == {"deal_cooling"}


def test_native_rule_ids_maps_each_capability_to_its_projected_rule_id():
    """The lifecycle logic must be right for the full roster regardless of what is scheduled now."""
    assert native_rule_ids((DEAL_COOLING_V1, DEAL_COOLING_FULL_V2)) == {
        "deal_cooling", "deal_cooling_full"}
...
def test_the_projection_is_a_total_function(orchestrator, publication):
    ...
    again = build_native_publication(
        execution=orchestrator.execute(_request(DELIVERABLE_V2)), audit_bundle=BUNDLE)
...
def test_a_capability_without_a_declared_cooldown_rests_for_its_own_expiry(orchestrator):
    ...
    quiet = replace(DELIVERABLE_V2, metadata={
        key: value for key, value in DELIVERABLE_V2.metadata.items()
