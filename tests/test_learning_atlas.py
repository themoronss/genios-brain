...
from genios_engine.feedback.governance import LearningPolicy, lifecycle_path, validate_learning
...
def learning(**changes) -> LearningObject:
    values = {
        "org_id": "org_1", "unit": LearningUnit.ADAPTIVE, "target": BrainTarget.ADAPTIVE,
...
def test_contract_has_no_expert_brain_target_and_knowledge_cannot_escape_review_target():
    ...
    with pytest.raises(ValueError, match="knowledge evolution"):
        learning(unit=LearningUnit.KNOWLEDGE, target=BrainTarget.ADAPTIVE)
...
def test_runtime_memory_is_always_leased_and_other_targets_cannot_smuggle_a_ttl():
    with pytest.raises(ValueError, match="requires expires_at"):
        learning(unit=LearningUnit.TEMPORARY_MEMORY, target=BrainTarget.RUNTIME)
...
def test_knowledge_evolution_only_suggests_review_after_a_sustained_poor_cohort():
    ...
    assert suggestion.target is BrainTarget.KNOWLEDGE_SUGGESTION
...
def test_preference_and_memory_units_refuse_silence_and_implicit_memory():
    ...
    assert len(memories) == 1 and memories[0].target is BrainTarget.RUNTIME
...
def test_all_ten_analysis_units_run_and_validation_is_the_eleventh_unit():
    ...
    deliveries = (DeliveryFact("d_1", "slack", "delivered", NOW - timedelta(seconds=1), NOW),)
...
def test_publisher_has_no_expert_branch_and_scheduler_wires_the_orchestrator():
    ...
    assert "BrainTarget.ORGANIZATION" in source
    assert "BrainTarget.BEHAVIOR" in source
    assert "BrainTarget.ADAPTIVE" in source
...
def test_orchestrator_claims_runs_applies_governance_and_reports_final_states(monkeypatch):
    ...
    completed = []
    monkeypatch.setattr(orchestrator, "expire_memories", lambda _c, _at: 2)
    monkeypatch.setattr(orchestrator, "claim_run", lambda _c, _o, _at: ("run_1", True))
    monkeypatch.setattr(orchestrator, "load_batch", lambda _c, _o, _at: LearningBatch("org_1", NOW))
    monkeypatch.setattr(orchestrator, "load_policy", lambda _c, _o: LearningPolicy())
    monkeypatch.setattr(orchestrator, "run_units", lambda _batch: items)
    monkeypatch.setattr(orchestrator, "persist_object", lambda _c, _i, _r: True)
    monkeypatch.setattr(orchestrator, "lifecycle_path", lambda _i, _p: ())
    final = iter((LearningState.PUBLISHED, LearningState.HUMAN_REVIEW))
    monkeypatch.setattr(orchestrator, "apply_path", lambda _c, _i, _p, _at: next(final))
