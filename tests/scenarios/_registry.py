...
SCENARIOS: dict[str, Scenario] = {s.id: s for s in (
    ...
    _s("S01b", "F05", "the pipeline feeds assemble_chain a whole thread", OPEN, "18",
       ...
       "did NOT need it. "
       "⛔ **PRICED 2026-09-25, AND IT STAYS OPEN AS A DECISION RATHER THAN AS AN OVERSIGHT.** "
       "Three routes exist and only one is correct. (a) Read siblings from `source_events`: needs "
       "a migration — there is no index on `(org_id, parent_object_id, occurred_at)` — plus a "
       "connection plumbed into `_thread_context`, which has none in scope, plus ONE READ PER "
       "THREADED EVENT on a module that today issues **one `.execute(` in its entirety**. The "
       "capture path is deliberately near-zero-I/O and this would change that structurally. "
       "(b) Read from `qualified_signals` by `thread_key` — the index already exists from step 18 "
       "and it is **WRONG**: signals are a subset of events (eight drop paths), so it answers "
       "'the last inbound we published a signal for', which is a different fact and exactly the "
       "quiet substitution this project refuses. (c) A thread object from the connector — none is "
       "stored; `source_objects` has no thread row. "
       "**WHAT IT BUYS: two benchmark objects, 24 → 26 of 38.** The seam is already correct — "
       "`_thread_context`'s own comment says so: *'passing them costs nothing and means the seam "
       "is already correct the day a caller supplies the sibling messages'* — so this is a "
       "supplier decision, not a code gap. **Rohit's call, priced: a structural change to the "
       "hottest path in the product for +2 of 38.**"),
